...
cat(paste("p value is", format(t1$p.value, scientific = TRUE, digits = 3)))
report::report(t1)
```
...
cat(paste("p value is", format(t2$p.value, scientific = TRUE, digits = 3)))
report::report(t2)
```
...
**Compute Z-Score for pathway `PS(42:8)` to `PE(42:8)` to `PC(42:8)`.**
...
**Compute Z-Score for pathway `PS(32:0)` to `PE(32:0)` to `PC(32:0)`.**
